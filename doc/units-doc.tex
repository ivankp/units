...
and perform all internal calculations without converting between kg and lb.
But if \lstinline{@FAddProjectile@()} is a part of the interface, the value that should be passed to
it may come from another library that made a different assumption and chose lb as base units instead
of kg. The situation may be as follows.
%
\begin{lstlisting}
...
\end{lstlisting}
%
But a software engineer who read the documentation for both libraries is the only one that can catch
this mistake.
The compiler cannot provide any help here.

The \lstinline{@Nunits@} library can help with this problem.
Exactly what is the best approach, depend on whether we own
\lstinline{@Nfoo@} and \lstinline{@Nbar@}.
...
\end{lstlisting}
%
Code becomes self-documenting and unit correctness is checked at compile time.
Note, that on the first line above \lstinline{"lb"@L_u@} is not a mistake.
The user needs to specify the units of the quantity that they have and the library handles the
conversion.
...
\end{lstlisting}
%
We divide by \lstinline{"kg"@L_u@} because we need the value in kg.
Here, again, code becomes self-documenting as units have to be explicitly stated, or it won't
compile.
